...
12. [デバッグモニターアプリ設計](#12-デバッグモニターアプリ設計)
13. [システムアーキテクチャ](#13-システムアーキテクチャ)
14. [将来拡張](#14-将来拡張)
...
| グループ切替 | 現フェーズはなし。グループIDはNVS（フラッシュ）に書き込んだ固定値を使用 |
...
| ステップ | 処理 | 失敗時 |
|----------|------|--------|
| 1-1 | NVS（フラッシュ）初期化 | 再起動 |
| 1-2 | NVSから設定読み込み（グループID・端末設定） | デフォルト値を使用 |
...
| `codec` | OpusエンコードAPI・デコードAPI・PLC | `opus_encode()`, `opus_decode()` |
...
| キー | 型 | 内容 | デフォルト |
|------|----|------|-----------|
| `group_id` | uint8 | 所属グループID | 1 |
| `server_ip` | string | VPSサーバーIPアドレス | — |
| `server_tcp_port` | uint16 | TCP制御ポート | 6000 |
| `server_udp_port` | uint16 | UDP音声ポート | 6001 |
...
- [x] グループ切替操作（現フェーズはNVS固定値、将来フェーズでボタン追加）
